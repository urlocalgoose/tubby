...
with open("oldSite.dat", "r", encoding = 'utf-8') as f:
   oldSite = f.read()

# Choose site at random that isnt the same as the one before
def chooseSite(lastSite):
    
    # get random item from sites list and make sure its not a repeat of last time
    item = random.choice(tuple(sites))
    if item == lastSite:
        item = chooseSite(lastSite)
    with open("oldSite.dat", 'w',encoding = 'utf-8') as f:
        f.write(item)
...
def scrapeController(site):
    if site == "https://allthatsinteresting.com/tag/history":
        content = interestingScrape(site)
    if site == "https://allthatsinteresting.com/tag/science":
        content = interestingScrape(site)
    if site == "https://www.reddit.com/r/WritingPrompts/":
        content = redditScrape(site)
    if site == "https://www.reddit.com/r/indepthstories/":
        content = redditScrape(site)
    if site == "https://getpocket.com/explore":
        content = getpocketScrape(site)
    if site == "https://medium.com/":
        content = mediumScrape(site)
...
def getData():
    #site = chooseSite(oldSite)
    #links = scrapeController(site)
